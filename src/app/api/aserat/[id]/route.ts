...
// GET handler - get single Aserat record
      ...
      if (error) {
        if (error.code === "PGRST116") {
          return NextResponse.json(
            { error: "Aserat record not found" } satisfies ApiError,
            { status: 404 },
          );
        }
...
// PATCH handler - record a payment toward an Aserat
...
const patchHandler = (routeParams: RouteParams): AuthenticatedHandler => {
  return async (req, user) => {
    try {
      ...
      // Calculate new amount_paid (accumulate, capped at amount_due)
      const newAmountPaid = Math.min(
        currentRecord.amount_paid + paymentAmount,
        currentRecord.amount_due,
      );
      ...
      if (wasNotPaid && updatedRecord.status === "paid") {
        ...
        await serviceClient.from("notifications").insert({
          user_id: user.id,
          type: "general",
          title: "Aserat Bekurat Complete",
          message: `You have completed your Aserat for ${currentRecord.month}/${currentRecord.year}. Well done!`,
          channel: "in_app",
          is_read: false,
